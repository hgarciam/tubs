...
#filename = '2020_data/CONTRAMOSTRES_2019-2020.xls'
#data_df = pd.read_excel(filename,decimal=",")
filename = '2020_data/CONTRAMOSTRES_2019-2020_SP.xls'
#data_df_SP = pd.read_excel(filename,decimal=",")
data_df_SP = pd.read_excel(filename)
#filename = '2020_data/CONTRAMOSTRES_2019-2020_TX.xls'
#data_df_TX = pd.read_excel(filename,decimal=",")
#filename = '2020_data/CONTRAMOSTRES_2019-2020_ACABAT.xls'
#data_df = pd.read_excel(filename,decimal=",")
...
print('A total of %1.2i (%1.2f%%) samples have been discarted' % (len(outliers_ID),len(outliers_ID)/len(data_df['OT'])*100))
...
if dataPlot:
    ...
    scatter_matrix(df, alpha=0.5, figsize=(12, 6), diagonal='hist')
    # Check options of scatter_matrix to increase bins and density plots
                                                                         
# Training
for i in [1]:
    X_train, X_test, y_train, y_test = train_test_split(polyX, y, test_size=0.1)
    print('Number of data points in train %i' % len(X_train))
    print('Number of data points in test %i' % len(X_test))
    if load_model:
        model_lasso = pickle.load(open('model_lasso.pkl', 'rb'))
        model_ridge = pickle.load(open('model_ridge.pkl', 'rb'))    
    else:
        model_lasso = Lasso(alpha=2e-3,normalize=False,max_iter=5e5).fit(X_train, y_train)
        model_ridge = Ridge(alpha=2e-3,normalize=False,max_iter=5e5).fit(X_train, y_train)
